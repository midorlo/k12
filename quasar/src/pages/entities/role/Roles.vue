<template>
  <div class="q-pa-md">
    <q-table
      :title="$t('k12App.role.home.title')"
      :rows="rows"
      :columns="columns"
      row-key="id"
    ...
    >
      <template v-slot:top-right>
        ...
        >
          <q-btn
            color="primary"
            :label="$t('k12App.role.home.createLabel')"
            @click="navigate"
          />
        </router-link>
      </template>
          ...
          <q-td>
            <div>
              <q-list dense>
                <q-item
                  :key="clearances.id"
                  v-for="clearances in props.row.clearances"
                >
                  <q-item-section>
                    {{ clearances.id }}
                  </q-item-section>
...
export default defineComponent({
  ...
  setup() {
    ...
    const columns = [
      { name: 'id', align: 'left', label: t('k12App.role.id'), field: 'id', sortable: true },
      { name: 'i18n', align: 'left', label: t('k12App.role.i18n'), field: 'i18n', sortable: true },
      { name: 'clearances.id', align: 'left', label: t('k12App.role.clearances'), field: 'clearances', sortable: false },
    ];
    ...
    return {
      ...
      deleteRole: id => {
        $q.dialog({
          message: t('k12App.role.delete.question', { id: id }),
          cancel: true,
        }).onOk(() => {
