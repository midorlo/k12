package com.midorlo.k12.config.web.advice;

import com.midorlo.k12.config.ApplicationConstants;
import com.midorlo.k12.config.web.advice.model.FieldErrorVM;
...
public class ExceptionTranslator implements ProblemHandling,
    ...
    public ResponseEntity<Problem> process(@Nullable ResponseEntity<Problem> entity,
        ...
        ProblemBuilder builder = Problem
            ...
            .withType(Problem.DEFAULT_TYPE.equals(problem.getType()) ?
                      ApplicationConstants.ErrorConstants.DEFAULT_TYPE : problem.getType())
        ...
        if (problem instanceof ConstraintViolationProblem) {
            ...
            builder.withCause(((ThrowableProblem) problem).getCause()).withDetail(problem.getDetail())
    ...
    public ResponseEntity<Problem> handleEmailAlreadyUsedException(
        ...
        return create(
            problem,
            request,
            RestUtilities.createFailureAlert(applicationName, problem.getEntityName(),
    ...
    @ExceptionHandler
    public ResponseEntity<Problem> handleBadRequestAlertException(BadRequestAlertProblem ex,
                                                                  NativeWebRequest request) {
        return create(
            ex,
            request,
            RestUtilities.createFailureAlert(applicationName, ex.getEntityName(), ex.getErrorKey(),
                                             ex.getMessage())
        );
    }
