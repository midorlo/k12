...
public class ApplicationMetricsEndpoint {
     ...
     */
    public ApplicationMetricsEndpoint(MeterRegistry meterRegistry) {
        this.meterRegistry = meterRegistry;
    }

    /**
     * GET /management/app-metrics
     * <p>
    ...
    private Map<String, Object> garbageCollectorMetrics() {
        ...
        timers.forEach(
            timer -> {
                ...
                Map<String, Number> gcPauseResults = new HashMap<>();
                ...
                crudOperation.forEach(
                    operation -> {
                        ...
                        if (count != 0) {
                            ...
                            resultsPerUriPerCrudOperation.put("max", getMax(httpTimersStream));
                            resultsPerUriPerCrudOperation.put("mean", getTotalTime(httpTimersStream) / count);
    ...
    private Map<String, Map<?, ?>> httpRequestsMetrics() {
        ...
        statusCode.forEach(
            code -> {
                ...
                resultsPerCode.put("mean", count != 0 ? getTotalTime(httpTimersStream) / count : 0);
